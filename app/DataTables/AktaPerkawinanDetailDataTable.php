...
class AktaPerkawinanDetailDataTable extends DataTable
{
    /**
            ...
            ->addColumn('action', function ($item) {
                ...
                if (PermissionCommon::check('usulan.approve_disdukcapil')) {
                    if ($item->submission->status == 1) {
                        $html .= '<br><button onclick="approve(\'' . $item->submission->uid . '\',\'akta_perkawinan\')" type="button" class="btn btn-sm bg-primary text-white mt-1" title="Approve Usulan"><i class="fas fa-clipboard-check"></i> Approve </button>';
                        $html .= '<button onclick="reject(\'' . $item->submission->uid . '\',\'akta_perkawinan\')" type="button" class="btn btn-sm bg-danger text-white mt-1" title="Tolak Usulan"><i class="fas fa-times-hexagon"></i> Tolak </button>';
                    }
     ...
     *
     * @param \App\Models\AktaPerkawinanDetail $model
    ...
    public function query(AktaPerkawinanDetail $model): QueryBuilder
    {
        if (str_contains(auth()->user()->role->slug, 'disdukcapil')) {
            ...
            return $model->newQuery()
                ...
                ->select('akta_perkawinan_details.*')
    ...
    public function html(): HtmlBuilder
    {
        ...
        if (PermissionCommon::check('akta_perkawinan.create')) {
            $button[] = Button::raw('<i class="fa fa-plus"></i> Tambah Usulan Penerbitan Akta Perkawinan')->action('function() { create() }');
        }
        return $this->builder()
            ->parameters([
                'language' => [
            ...
            ->setTableId('aktaperkawinandetail-table')
    ...
    public function getColumns(): array
    {
        ...
        $column[] = Column::make('tanggal_pemberkatan')->title('Tanggal Pemberkatan');
        $column[] = Column::make('tempat_pemberkatan')->title('Tempat Pemberkatan');
        return $column;
    }
    ...
    {
        return 'AktaPerkawinanDetail_' . date('YmdHis');
    }
}
