...
class PembatalanPerkawinanDetailDataTable extends DataTable
{
    /**
            ...
            ->orderColumn('no_perkara', function ($query, $direction) {
                $query->orderBy(
                    Submission::select('no_perkara')
                        ->whereColumn('submissions.uid', 'pembatalan_perkawinan_details.submission_uid')
                        ->limit(1),
                    $direction
                );
            })
     ...
     *
     * @param \App\Models\PembatalanPerkawinanDetail $model
     * @return \Illuminate\Database\Eloquent\Builder
     */
    public function query(PembatalanPerkawinanDetail $model): QueryBuilder
    {
    ...
    public function html(): HtmlBuilder
    {
        ...
        return $this->builder()
            ...
            ->setTableId('pembatalanperkawinandetail-table')
    ...
    {
        $column = [];
        if (PermissionCommon::check('pembatalan_perkawinan.update') || PermissionCommon::check('pembatalan_perkawinan.delete') || PermissionCommon::check('usulan.approve_disdukcapil')) {
    ...
    {
        return 'PembatalanPerkawinanDetail_' . date('YmdHis');
    }
}
