...
class AktaPerceraianDetailDataTable extends DataTable
{
    /**
            ...
            ->addColumn('action', function ($item) {
                ...
                if (PermissionCommon::check('usulan.approve_disdukcapil')) {
                    if ($item->submission->status == 1) {
                        $html .= '<br><button onclick="approve(\'' . $item->submission->uid . '\',\'akta_perceraian\')" type="button" class="btn btn-sm bg-primary text-white mt-1" title="Approve Usulan"><i class="fas fa-clipboard-check"></i> Approve </button>';
                        $html .= '<button onclick="reject(\'' . $item->submission->uid . '\',\'akta_perceraian\')" type="button" class="btn btn-sm bg-danger text-white mt-1" title="Tolak Usulan"><i class="fas fa-times-hexagon"></i> Tolak </button>';
                    }
            ...
            ->orderColumn('no_perkara', function ($query, $direction) {
                $query->orderBy(
                    Submission::select('no_perkara')
                        ->whereColumn('submissions.uid', 'akta_perceraian_details.submission_uid')
                        ->limit(1),
                    $direction
                );
            })
     ...
     *
     * @param \App\Models\AktaPerceraianDetail $model
     * @return \Illuminate\Database\Eloquent\Builder
     */
    public function query(AktaPerceraianDetail $model): QueryBuilder
    {
    ...
    public function html(): HtmlBuilder
    {
        ...
        return $this->builder()
            ...
            ->setTableId('aktaperceraiandetail-table')
    ...
    {
        $column = [];
        if (PermissionCommon::check('akta_perceraian.update') || PermissionCommon::check('akta_perceraian.delete') || PermissionCommon::check('usulan.approve_disdukcapil')) {
        ...
        $column[] = Column::make('nama_suami')->title('Nama Suami');
        $column[] = Column::make('nama_istri')->title('Nama Istri');
        $column[] = Column::make('sebab_perceraian')->title('Sebab Perceraian');
        return $column;
    }
    ...
    {
        return 'AktaPerceraianDetail_' . date('YmdHis');
    }
}
