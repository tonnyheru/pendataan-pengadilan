...
class PengangkatanAnakDetailDataTable extends DataTable
{
    /**
            ...
            ->addColumn('action', function ($item) {
                ...
                if (PermissionCommon::check('usulan.approve_disdukcapil')) {
                    if ($item->submission->status == 1) {
                        $html .= '<br><button onclick="approve(\'' . $item->submission->uid . '\',\'pengangkatan_anak\')" type="button" class="btn btn-sm bg-primary text-white mt-1" title="Approve Usulan"><i class="fas fa-clipboard-check"></i> Approve </button>';
                        $html .= '<button onclick="reject(\'' . $item->submission->uid . '\',\'pengangkatan_anak\')" type="button" class="btn btn-sm bg-danger text-white mt-1" title="Tolak Usulan"><i class="fas fa-times-hexagon"></i> Tolak </button>';
                    }
            ...
            ->orderColumn('no_perkara', function ($query, $direction) {
                $query->orderBy(
                    Submission::select('no_perkara')
                        ->whereColumn('submissions.uid', 'pengangkatan_anak_details.submission_uid')
                        ->limit(1),
                    $direction
                );
            })
     ...
     *
     * @param \App\Models\PengangkatanAnakDetail $model
     * @return \Illuminate\Database\Eloquent\Builder
     */
    public function query(PengangkatanAnakDetail $model): QueryBuilder
    {
        if (str_contains(auth()->user()->role->slug, 'disdukcapil')) {
            ...
            return $model->newQuery()
                ->join('submissions', 'submissions.uid', '=', 'pengangkatan_anak_details.submission_uid')
                ->select('pengangkatan_anak_details.*')
    ...
    public function html(): HtmlBuilder
    {
        ...
        if (PermissionCommon::check('pengangkatan_anak.create')) {
            $button[] = Button::raw('<i class="fa fa-plus"></i> Tambah Usulan Pengangkatan Anak')->action('function() { create() }');
        }
        return $this->builder()
            ->parameters([
                'language' => [
            ...
            ->setTableId('pengangkatananakdetail-table')
    ...
    {
        $column = [];
        if (PermissionCommon::check('pengangkatan_anak.update') || PermissionCommon::check('pengangkatan_anak.delete') || PermissionCommon::check('usulan.approve_disdukcapil')) {
    ...
    {
        return 'PengangkatanAnakDetail_' . date('YmdHis');
    }
}
