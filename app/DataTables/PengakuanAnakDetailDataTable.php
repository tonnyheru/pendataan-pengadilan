...
class PengakuanAnakDetailDataTable extends DataTable
{
    /**
            ...
            ->addColumn('action', function ($item) {
                ...
                if (PermissionCommon::check('usulan.approve_disdukcapil')) {
                    if ($item->submission->status == 1) {
                        $html .= '<br><button onclick="approve(\'' . $item->submission->uid . '\',\'pengakuan_anak\')" type="button" class="btn btn-sm bg-primary text-white mt-1" title="Approve Usulan"><i class="fas fa-clipboard-check"></i> Approve </button>';
                        $html .= '<button onclick="reject(\'' . $item->submission->uid . '\',\'pengakuan_anak\')" type="button" class="btn btn-sm bg-danger text-white mt-1" title="Tolak Usulan"><i class="fas fa-times-hexagon"></i> Tolak </button>';
                    }
            ...
            ->orderColumn('no_perkara', function ($query, $direction) {
                $query->orderBy(
                    Submission::select('no_perkara')
                        ->whereColumn('submissions.uid', 'pengakuan_anak_details.submission_uid')
                        ->limit(1),
                    $direction
                );
            })
     ...
     *
     * @param \App\Models\PengakuanAnakDetail $model
     * @return \Illuminate\Database\Eloquent\Builder
     */
    public function query(PengakuanAnakDetail $model): QueryBuilder
    {
    ...
    public function html(): HtmlBuilder
    {
        ...
        return $this->builder()
            ...
            ->setTableId('pengakuananakdetail-table')
    ...
    public function getColumns(): array
    {
        ...
        $column = [];
        if (PermissionCommon::check('pengakuan_anak.update') || PermissionCommon::check('pengakuan_anak.delete') || PermissionCommon::check('usulan.approve_disdukcapil')) {
        ...
        $column[] = Column::make('tipe')->title("Pengesahan / Pengakuan");
        return $column;
    }
    ...
    {
        return 'PengakuanAnakDetail_' . date('YmdHis');
    }
}
