...
class PembatalanAktaKelahiranDetailDataTable extends DataTable
{
    /**
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', function ($item) {
                ...
                if (PermissionCommon::check('pembatalan_akta_kelahiran.update')) {
                    $html .= '<button onclick="edit(\'' . $item->uid . '\')" type="button" class="btn btn-sm btn-info" title="Edit"><i class="fas fa-pen"></i></button>';
                }
                if (PermissionCommon::check('pembatalan_akta_kelahiran.delete')) {
                ...
                if (PermissionCommon::check('usulan.approve_disdukcapil')) {
                    if ($item->submission->status == 1) {
                        $html .= '<br><button onclick="approve(\'' . $item->submission->uid . '\',\'pembatalan_akta_kelahiran\')" type="button" class="btn btn-sm bg-primary text-white mt-1" title="Approve Usulan"><i class="fas fa-clipboard-check"></i> Approve </button>';
                        $html .= '<button onclick="reject(\'' . $item->submission->uid . '\',\'pembatalan_akta_kelahiran\')" type="button" class="btn btn-sm bg-danger text-white mt-1" title="Tolak Usulan"><i class="fas fa-times-hexagon"></i> Tolak </button>';
                    }
            ...
            ->orderColumn('no_perkara', function ($query, $direction) {
                $query->orderBy(
                    Submission::select('no_perkara')
                        ->whereColumn('submissions.uid', 'pembatalan_akta_kelahiran_details.submission_uid')
                        ->limit(1),
                    $direction
                );
            })
     ...
     *
     * @param \App\Models\PembatalanAktaKelahiranDetail $model
     * @return \Illuminate\Database\Eloquent\Builder
     */
    public function query(PembatalanAktaKelahiranDetail $model): QueryBuilder
    {
        if (str_contains(auth()->user()->role->slug, 'disdukcapil')) {
            ...
            return $model->newQuery()
                ->join('submissions', 'submissions.uid', '=', 'pembatalan_akta_kelahiran_details.submission_uid')
                ->select('pembatalan_akta_kelahiran_details.*')
    ...
    public function html(): HtmlBuilder
    {
        ...
        if (PermissionCommon::check('pembatalan_akta_kelahiran.create')) {
            $button[] = Button::raw('<i class="fa fa-plus"></i> Tambah Usulan Pembatalan Akta Kelahiran')->action('function() { create() }');
        }
        return $this->builder()
            ->parameters([
                'language' => [
            ...
            ->setTableId('pembatalanaktakelahirandetail-table')
    ...
    {
        $column = [];
        if (PermissionCommon::check('pembatalan_akta_kelahiran.update') || PermissionCommon::check('pembatalan_akta_kelahiran.delete') || PermissionCommon::check('usulan.approve_disdukcapil')) {
        ...
        $column[] = Column::make('nama_pemilik_akta');
        return $column;
    }
    ...
    {
        return 'PembatalanAktaKelahiranDetail_' . date('YmdHis');
    }
}
