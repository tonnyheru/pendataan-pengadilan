...
class AktaKematianDetailDataTable extends DataTable
{
    /**
            ...
            ->addColumn('action', function ($item) {
                ...
                if (PermissionCommon::check('usulan.approve_disdukcapil')) {
                    if ($item->submission->status == 1) {
                        $html .= '<br><button onclick="approve(\'' . $item->submission->uid . '\',\'akta_kematian\')" type="button" class="btn btn-sm bg-primary text-white mt-1" title="Approve Usulan"><i class="fas fa-clipboard-check"></i> Approve </button>';
                        $html .= '<button onclick="reject(\'' . $item->submission->uid . '\',\'akta_kematian\')" type="button" class="btn btn-sm bg-danger text-white mt-1" title="Tolak Usulan"><i class="fas fa-times-hexagon"></i> Tolak </button>';
                    }
            ...
            ->orderColumn('no_perkara', function ($query, $direction) {
                $query->orderBy(
                    Submission::select('no_perkara')
                        ->whereColumn('submissions.uid', 'akta_kematian_details.submission_uid')
                        ->limit(1),
                    $direction
                );
            })
     ...
     *
     * @param \App\Models\AktaKematianDetail $model
    ...
    public function html(): HtmlBuilder
    {
        ...
        return $this->builder()
            ...
            ->setTableId('aktakematiandetail-table')
    ...
    public function getColumns(): array
    {
        ...
        $column[] = Column::make('nik_jenazah')->title('NIK Jenazah');
        $column[] = Column::make('nama_jenazah')->title('Nama Jenazah');
        $column[] = Column::make('tanggal_kematian')->title('Tanggal Kematian');
        return $column;
    }
    ...
    {
        return 'AktaKematianDetail_' . date('YmdHis');
    }
}
