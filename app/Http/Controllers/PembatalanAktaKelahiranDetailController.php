...
use App\DataTables\PembatalanAktaKelahiranDetailDataTable;
use App\Helpers\PermissionCommon;
use App\Helpers\WhatsappHelper;
use App\Mail\NotifEmail;
use App\Models\Disdukcapil;
use App\Models\PembatalanAktaKelahiranDetail;
...
class PembatalanAktaKelahiranDetailController extends Controller
{
    /**
     * Display a listing of the resource.
     */
    public function index(PembatalanAktaKelahiranDetailDataTable $dataTable)
    {
        if (!PermissionCommon::check('pembatalan_akta_kelahiran.list')) abort(403);
        return $dataTable->render('pages.administrasi.usulan.pembatalan_akta_kelahiran.list');
    }
    ...
    {
        if (!PermissionCommon::check('pembatalan_akta_kelahiran.create')) abort(403);
        $pemohon = Pemohon::all();
        $disdukcapil = Disdukcapil::all();
        $body = view('pages.administrasi.usulan.pembatalan_akta_kelahiran.create', compact('pemohon', 'disdukcapil'))->render();
        $footer = '<button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
                    <button type="button" class="btn btn-primary" onclick="save()">Save</button>';

        return [
            'title' => 'Tambah Usulan Pembatalan Akta Kelahiran',
            'body' => $body,
            'footer' => $footer
        ];
    }
    ...
    public function store(Request $request)
    {
        ...
        $request->validate([
            ...
            'file_keabsahan_akta_kelahiran' => 'required|mimes:jpg,jpeg,png,gif,pdf|max:2048',
        ], [
            ...
            'nama_pemilik_akta.required' => 'Nama Pemilik Akta tidak boleh kosong',
            ...
            'file_keabsahan_akta_kelahiran.required' => 'File Keabsahan Akta Kelahiran tidak boleh kosong',
            'file_keabsahan_akta_kelahiran.mimes' => 'Format file harus JPG, PNG, atau PDF',
            'file_keabsahan_akta_kelahiran.max' => 'Ukuran file keabsahan akta kelahiran maksimal 2MB',
        ...
        try {
            $submission = Submission::create([
                ...
                'submission_type' => 'pembatalan_akta_kelahiran',
            ...
            if ($request->hasFile('keabsahan_akta_kelahiran')) {
                ...
                $file_keabsahan_name = md5('keabsahan_akta_kelahiran' . time()) . time() . '.' . $file_keabsahan->getClientOriginalExtension();
                $data['path_keabsahan_akta_kelahiran'] = $file_keabsahan_name;
                $documents[] = [
                    'uid' => Str::uuid()->toString(),
                    'submission_uid' => $submission->uid,
                    'document_name' => $file_keabsahan->getClientOriginalName(),
                    'document_type' => 'keabsahan_akta_kelahiran',
            ...
            $trx = PembatalanAktaKelahiranDetail::create([
                'uid' => Str::uuid()->toString(),
                'submission_uid' => $submission->uid,
                'nama_pemilik_akta' => $data['nama_pemilik_akta'],
            ]);
            ...
            if ($trx) {
                ...
                if ($disdukcapil) {
                    ...
                    $notif['jenis_perkara'] = "Pembatalan Akta Kelahiran";
                    ...
                    $jenis_permohonan = "Pembatalan Akta Kelahiran";
                    $message = <<<EOT
     ...
     */
    public function show(PembatalanAktaKelahiranDetail $pembatalanAktaKelahiranDetail)
    {
        //
    }
    ...
    {
        if (!PermissionCommon::check('pembatalan_akta_kelahiran.update')) abort(403);
        $pembatalanAktaKelahiranDetail = PembatalanAktaKelahiranDetail::find($uid);
        if ($pembatalanAktaKelahiranDetail) {
            $uid = $pembatalanAktaKelahiranDetail->uid;
            $data = $pembatalanAktaKelahiranDetail;
            $pemohon = Pemohon::all();
            $disdukcapil = Disdukcapil::all();
            $body = view('pages.administrasi.usulan.pembatalan_akta_kelahiran.edit', compact('uid', 'data', 'pemohon', 'disdukcapil'))->render();
            $footer = '<button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
                <button type="button" class="btn btn-primary" onclick="save()">Save</button>';
            return [
                'title' => 'Edit Usulan Pembatalan Akta Kelahiran',
                'body' => $body,
                'footer' => $footer
            ];
        } else {
    ...
    public function update(Request $request, $uid)
    {
        ...
        $pembatalanAktaKelahiranDetail = PembatalanAktaKelahiranDetail::find($uid);
        $request->validate([
            'no_perkara' => "required|unique:submissions,no_perkara," . $pembatalanAktaKelahiranDetail->submission->uid . ",uid",
            'pemohon_uid' => 'required',
            'disdukcapil' => 'required',
            ...
            'file_keabsahan_akta_kelahiran' => 'mimes:jpg,jpeg,png,gif,pdf|max:2048',
        ], [
            'no_perkara.required' => 'Nomor Perkara tidak boleh kosong',
            'no_perkara.unique' => 'Nomor Perkara sudah terdaftar',
            'pemohon_uid.required' => 'Pemohon tidak boleh kosong',
        ...
        try {
            $documents = SubmissionDocument::where('submission_uid', $pembatalanAktaKelahiranDetail->submission->uid)->get();
            ...
            $name = auth()->user()->name;
            $role = auth()->user()->role->name;
            $catatan = json_decode($pembatalanAktaKelahiranDetail->submission->catatan);
            $catatan[] = [
                'role' => $role,
                'name' => $name,
                'status' => '4',
            ...
            $pembatalanAktaKelahiranDetail->submission->no_perkara = $formData['no_perkara'];
            $pembatalanAktaKelahiranDetail->submission->pemohon_uid = $formData['pemohon_uid'];
            $pembatalanAktaKelahiranDetail->submission->disdukcapil_uid = $formData['disdukcapil'];
            $pembatalanAktaKelahiranDetail->submission->status = '1';
            $pembatalanAktaKelahiranDetail->submission->catatan = json_encode($catatan);
            $pembatalanAktaKelahiranDetail->submission->updated_by = auth()->user()->uid;
            $pembatalanAktaKelahiranDetail->submission->save();


            $pembatalanAktaKelahiranDetail->nama_pemilik_akta = $formData['nama_pemilik_akta'];

            $trx = $pembatalanAktaKelahiranDetail->save();
    ...
    {
        if (!PermissionCommon::check('pembatalan_akta_kelahiran.delete')) abort(403);
        $pembatalanAktaKelahiranDetail = PembatalanAktaKelahiranDetail::find($uid);
        if (!$pembatalanAktaKelahiranDetail) {
            return response([
                'status' => false,
        ...
        try {
            $documents = $pembatalanAktaKelahiranDetail->submission->documents;
            foreach ($documents as $document) {
                // Delete the old profile image if it exists
                if ($document && file_exists(public_path("upload/file_$document->document_type/" . $document->file_path))) {
                    unlink(public_path("upload/file_$document->document_type/" . $document->file_path));
            ...
            $pembatalanAktaKelahiranDetail->delete();
            $pembatalanAktaKelahiranDetail->submission->documents()->delete();
            $pembatalanAktaKelahiranDetail->submission->delete();
            return response([
                'status' => true,
    ...
    public function showCatatan($uid)
    {
        try {
            ...
            if ($pembatalanAktaKelahiranDetail) {
                $catatan = json_decode($pembatalanAktaKelahiranDetail->submission->catatan);
                $body = view('pages.administrasi.usulan.pembatalan_akta_kelahiran.catatan', compact('pembatalanAktaKelahiranDetail', 'catatan'))->render();
                $footer = '<button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>';
                return [
                    'title' => 'Lihat Catatan',
                    'body' => $body,
