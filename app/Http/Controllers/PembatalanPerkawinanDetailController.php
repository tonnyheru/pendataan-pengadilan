...
use App\DataTables\PembatalanPerkawinanDetailDataTable;
use App\Helpers\PermissionCommon;
use App\Helpers\WhatsappHelper;
use App\Mail\NotifEmail;
use App\Models\Disdukcapil;
use App\Models\PembatalanPerkawinanDetail;
...
class PembatalanPerkawinanDetailController extends Controller
{
    /**
     * Display a listing of the resource.
     */
    public function index(PembatalanPerkawinanDetailDataTable $dataTable)
    {
        if (!PermissionCommon::check('pembatalan_perkawinan.list')) abort(403);
        return $dataTable->render('pages.administrasi.usulan.pembatalan_perkawinan.list');
    }
    ...
    {
        if (!PermissionCommon::check('pembatalan_perkawinan.create')) abort(403);
        $pemohon = Pemohon::all();
        $disdukcapil = Disdukcapil::all();
        $body = view('pages.administrasi.usulan.pembatalan_perkawinan.create', compact('pemohon', 'disdukcapil'))->render();
        $footer = '<button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
                    <button type="button" class="btn btn-primary" onclick="save()">Save</button>';

        return [
            'title' => 'Tambah Usulan Pembatalan Perkawinan',
            'body' => $body,
            'footer' => $footer
        ];
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            $submission = Submission::create([
                ...
                'submission_type' => 'pembatalan_perkawinan',
            ...
            $trx = PembatalanPerkawinanDetail::create([
                'uid' => Str::uuid()->toString(),
                'submission_uid' => $submission->uid,
            ...
            if ($trx) {
                ...
                if ($disdukcapil) {
                    ...
                    $notif['jenis_perkara'] = "Pembatalan Akta Perkawinan";
                    ...
                    $jenis_permohonan = "Pembatalan Akta Perkawinan";
                    $message = <<<EOT
                    Yth. $disdukcapil,
     ...
     */
    public function show(PembatalanPerkawinanDetail $pembatalanPerkawinanDetail)
    {
        //
    }
    ...
    {
        if (!PermissionCommon::check('pembatalan_perkawinan.update')) abort(403);
        $pembatalanPerkawinanDetail = PembatalanPerkawinanDetail::find($uid);
        if ($pembatalanPerkawinanDetail) {
            $uid = $pembatalanPerkawinanDetail->uid;
            $data = $pembatalanPerkawinanDetail;
            $pemohon = Pemohon::all();
            $disdukcapil = Disdukcapil::all();
            $body = view('pages.administrasi.usulan.pembatalan_perkawinan.edit', compact('uid', 'data', 'pemohon', 'disdukcapil'))->render();
            $footer = '<button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
                <button type="button" class="btn btn-primary" onclick="save()">Save</button>';
            return [
                'title' => 'Edit Usulan Pembatalan Perkawinan',
                'body' => $body,
                'footer' => $footer
            ];
        } else {
    ...
    public function update(Request $request, $uid)
    {
        ...
        $pembatalanPerkawinanDetail = PembatalanPerkawinanDetail::find($uid);
        $request->validate([
            'no_perkara' => "required|unique:submissions,no_perkara," . $pembatalanPerkawinanDetail->submission->uid . ",uid",
            'pemohon_uid' => 'required',
            'disdukcapil' => 'required',
        ...
        try {
            $documents = SubmissionDocument::where('submission_uid', $pembatalanPerkawinanDetail->submission->uid)->get();
            ...
            $name = auth()->user()->name;
            $role = auth()->user()->role->name;
            $catatan = json_decode($pembatalanPerkawinanDetail->submission->catatan);
            $catatan[] = [
                'role' => $role,
                'name' => $name,
                'status' => '4',
            ...
            $pembatalanPerkawinanDetail->submission->no_perkara = $formData['no_perkara'];
            $pembatalanPerkawinanDetail->submission->pemohon_uid = $formData['pemohon_uid'];
            $pembatalanPerkawinanDetail->submission->disdukcapil_uid = $formData['disdukcapil'];
            $pembatalanPerkawinanDetail->submission->status = '1';
            $pembatalanPerkawinanDetail->submission->catatan = json_encode($catatan);
            $pembatalanPerkawinanDetail->submission->updated_by = auth()->user()->uid;
            $pembatalanPerkawinanDetail->submission->save();


            $pembatalanPerkawinanDetail->nama_suami = $formData['nama_suami'];
            $pembatalanPerkawinanDetail->nama_istri = $formData['nama_istri'];

            $trx = $pembatalanPerkawinanDetail->save();
    ...
    {
        if (!PermissionCommon::check('pembatalan_perkawinan.delete')) abort(403);
        $pembatalanPerkawinanDetail = PembatalanPerkawinanDetail::find($uid);
        if (!$pembatalanPerkawinanDetail) {
            return response([
                'status' => false,
        ...
        try {
            $documents = $pembatalanPerkawinanDetail->submission->documents;
            foreach ($documents as $document) {
                // Delete the old profile image if it exists
                if ($document && file_exists(public_path("upload/file_$document->document_type/" . $document->file_path))) {
                    unlink(public_path("upload/file_$document->document_type/" . $document->file_path));
            ...
            $pembatalanPerkawinanDetail->delete();
            $pembatalanPerkawinanDetail->submission->documents()->delete();
            $pembatalanPerkawinanDetail->submission->delete();
            return response([
                'status' => true,
    ...
    public function showCatatan($uid)
    {
        try {
            ...
            if ($pembatalanPerkawinanDetail) {
                $catatan = json_decode($pembatalanPerkawinanDetail->submission->catatan);
                $body = view('pages.administrasi.usulan.pembatalan_perkawinan.catatan', compact('pembatalanPerkawinanDetail', 'catatan'))->render();
                $footer = '<button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>';
                return [
                    'title' => 'Lihat Catatan',
                    'body' => $body,
