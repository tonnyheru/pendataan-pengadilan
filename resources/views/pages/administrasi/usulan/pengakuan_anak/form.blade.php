...
<div class="row">
  ...
  <div class="col-md-12 justify-content-start">
    <h5 style="color: #016004">Data Anak</h5>
    <hr class="bg-diy"style="height: 2px; margin-top: 0px !important; margin-bottom: 10px !important;">
  </div>

  <div class="form-group col-md-12">
    <label>Nama Anak <span class="text-danger">*</span></label>
    <input type="text" name="nama_anak" class="form-control" placeholder="Nama Anak" value="{{ @$data->nama_anak }}">
  </div>

  <div class="form-group col-md-12 tipe">
    <label>Pengesahan / Pengakuan <span class="text-danger">*</span></label>
    <select name="tipe" class="form-control select2-tipe">
      <option value="pengesahan" @if(@$data->tipe == 'pengesahan') selected @endif>Pengesahan</option>
      <option value="pengakuan" @if(@$data->tipe == 'pengakuan') selected @endif>Pengakuan</option>
  ...
  $dokumen = [
    ...
    'kk_orangtua' => 'Kartu Keluarga Orang Tua',
    'ktp_pemohon' => 'KTP Pemohon',
  ];

  @endphp
  @foreach($dokumen as $key => $dok)
    <div class="col-md-3" id="group_{{ $key }}">
...
<script>
  ...
  // Tampilkan dokumen berdasarkan tipe
  function toggleDokumenByTipe(tipe) {
    const allDokumen = new Set([...dokumenPengesahan, ...dokumenPengakuan]);
    allDokumen.forEach(dok => {
      $(`#group_${dok}`).hide();
    });

    let dokumenToShow = [];
    if (tipe === 'pengesahan') {
      dokumenToShow = dokumenPengesahan;
    } else if (tipe === 'pengakuan') {
      dokumenToShow = dokumenPengakuan;
    }

    dokumenToShow.forEach(dok => {
      $(`#group_${dok}`).show();
    });
  }

  // Inisialisasi awal jika ada nilai default
  const initialTipe = $('select[name="tipe"]').val();
  toggleDokumenByTipe(initialTipe);

  // Event listener ketika tipe berubah
  $('select[name="tipe"]').on('change', function () {
    const tipe = $(this).val();
    toggleDokumenByTipe(tipe);
  });
